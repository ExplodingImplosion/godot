...
Array set_serialization_properties(Array serialization_properties);
Array get_nodes();
Array set_nodes(Array nodes);
Dictionary get_test_dict();
Dictionary set_test_dict(Dictionary test_dict);
float get_update_frequency();
float set_update_frequency(float update_frequency);
float get_update_time_left();
float set_update_time_left(float update_time_left);
bool get_updating();
bool set_updating(bool updating);
int get_uid();
int set_uid(int uid);
int get_scene_id();
int set_scene_id(int scene_id);
bool get_has_serialized();
bool set_has_serialized(bool has_serialized);
RefCounted get_serialized();
RefCounted set_serialized(RefCounted serialized);
};
