...
bool get_owner();
bool set_owner(bool p_owner);
String get_owner_scene_path();
String set_owner_scene_path(String p_owner_scene_path);
int get_num_props();
int set_num_props(int p_num_props);
Script get_source_script();
Script set_source_script(Script p_source_script);
StringName get_base_class();
StringName set_base_class(StringName p_base_class);
Array get_properties();
Array set_properties(Array p_properties);
PackedByteArray get_iter_all();
PackedByteArray set_iter_all(PackedByteArray p_iter_all);
PackedByteArray get_iter_owner_only();
PackedByteArray set_iter_owner_only(PackedByteArray p_iter_owner_only);
PackedByteArray get_iter_team_only();
PackedByteArray set_iter_team_only(PackedByteArray p_iter_team_only);
PackedByteArray get_iter_physical();
PackedByteArray set_iter_physical(PackedByteArray p_iter_physical);
Array get_iters();
Array set_iters(Array p_iters);
Array get_iters_visible();
Array set_iters_visible(Array p_iters_visible);
Dictionary get_bitmask_map();
Dictionary set_bitmask_map(Dictionary p_bitmask_map);
bool get_is_fixed_size();
bool set_is_fixed_size(bool p_is_fixed_size);
int get_num_bools();
int set_num_bools(int p_num_bools);
int get_fixed_size_bytes();
int set_fixed_size_bytes(int p_fixed_size_bytes);
GdScript get_type();
GdScript set_type(GdScript p_type);
Callable get_get_plist_func();
Callable set_get_plist_func(Callable p_get_plist_func);
Callable get_get_plist_short_func();
Callable set_get_plist_short_func(Callable p_get_plist_short_func);
};
